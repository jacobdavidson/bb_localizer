...
def data_based_init(model, input):
    ...
    if type(input) is dict:
        ...
        feed_dict = {
            tf_inp: np_inp
            for tf_inp, np_inp in zip(model.inputs, input)
        }
    else:
    ...
    for l, o, W, b in layer_output_weight_bias:
        ...
        s = tf.sqrt(v + 1e-10)
        updates = tf.group(
            W.assign(W / tf.reshape(s, [1] * (len(W.get_shape()) - 1) + [-1])),
            b.assign((b - m) / s))
...
class SGDWithWeightnorm(SGD):
    def get_updates(self, loss, params):
        ...
        if self.initial_decay > 0:
            lr *= (
                1. / (1. + self.decay * K.cast(self.iterations, K.floatx())))
    ...
    def get_updates(self, loss, params):
        ...
        for p, g, m, v in zip(params, grads, ms, vs):
            ...
            if len(ps) > 1:
                ...
                v_g_t = (self.beta_2 * v_g) + \
                    (1. - self.beta_2) * K.square(grad_g)
                new_g_param = g_param - lr_t * m_g_t / \
                    (K.sqrt(v_g_t) + self.epsilon)
